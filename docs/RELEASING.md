...
The installer uses authenticated GitHub release downloads from `altonwells/x-bot-follower-remover`. Release checksums provide download-integrity verification; trust comes from access to the private GitHub repository, not an independent signing key.
...
5. Confirm the repository is private with `gh repo view altonwells/x-bot-follower-remover --json isPrivate`.
